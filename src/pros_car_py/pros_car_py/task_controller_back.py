...
class TaskController:
    ...
    GRAB_CAMERA_DISTANCE_THRESHOLD = 0.395
    # 只有在目標已經置中時才允許前進靠近。
    # 這避免車子斜著靠近，導致手臂最後偏抓。
    ...
    def __init__(
        ...
        self._log_event("TaskController yesinitialized")
    ...
    def _search_target(self):
        # 看 /yolo/target_info 裡面的第一個值是不是 1 -> 沒有就原地慢慢轉
    ...
    def _approach_target(self):
        ...
        # 1. 看不到目標：短暫容錯count++8，但不要使用舊資料直接 observe / grab。
        """
        1. 沒看到目標
        ↓
        先停一下等 YOLO 回來
        ↓
        如果連續太多次8都沒看到，就回 SEARCH_TARGET 重新找
        """
    ...
    def _grab_target(self):
        ...
        # 抓取流程結束後先停掉 waving / auto thread，抬回不擋相機的位置。
        # look here (要新增一個抓著熊回家 不擋視線的或是沒差 回家的手臂狀態)
        self._move_arm_to_vision_pose("grab_wait_complete")

        self._log_event("grab_wait_complete returning_home")
        # look here
        self._transition(TaskState.RETURN_HOME, "Returning to home pose.")

        return "STOP"

    def _return_home(self):#look here 少一個回家後把雄放下的狀態
    ...
    def _is_forward_action(self, action):
        return action in ("FORWARD_SLOW", "FORWARD_VERY_SLOW")
    ...
    def _apply_obstacle_guard(self, action, target_info=None, context=""):
        ...
        if not depths:
            self._log_event(
                f"obstacle_guard no_depth_data context={context} raw_action={action}"
            )
            self._debug(
                f"obstacle_guard no_depth_data context={context} raw_action={action}",
                1.0,
        ...
        if center_depth is None:
            self._log_event(
                f"obstacle_guard no_center_depth context={context} raw_action={action}"
            )
            self._debug(
                f"obstacle_guard no_center_depth context={context} raw_action={action}",
                1.0,
        ...
        if center_depth <= self.OBSTACLE_EMERGENCY_STOP_DISTANCE:
            self._log_event(
                f"obstacle_guard emergency_stop context={context} center_depth={center_depth:.3f} raw_action={action}"
            )
            self._debug(
                f"obstacle_guard emergency_stop context={context} center_depth={center_depth:.3f}",
                0.5,
            )
            return "STOP"

        likely_target = self._is_center_depth_likely_target(center_depth, target_info)

        if center_depth <= self.OBSTACLE_FRONT_STOP_DISTANCE and not likely_target:
            safe_action = "STOP"
            if left_depth is not None and right_depth is not None:
                if left_depth > right_depth and left_depth >= self.OBSTACLE_SIDE_CLEAR_DISTANCE:
                    safe_action = "COUNTERCLOCKWISE_ROTATION_FINE"
                elif right_depth >= self.OBSTACLE_SIDE_CLEAR_DISTANCE:
                    safe_action = "CLOCKWISE_ROTATION_FINE"
            elif left_depth is not None and left_depth >= self.OBSTACLE_SIDE_CLEAR_DISTANCE:
                safe_action = "COUNTERCLOCKWISE_ROTATION_FINE"
            elif right_depth is not None and right_depth >= self.OBSTACLE_SIDE_CLEAR_DISTANCE:
                safe_action = "CLOCKWISE_ROTATION_FINE"

            self._log_event(
                "obstacle_guard block_forward "
                f"context={context} center_depth={self._format_float(center_depth, 3)} "
                f"left_depth={self._format_float(left_depth, 3)} "
                f"right_depth={self._format_float(right_depth, 3)} "
                f"likely_target={likely_target} raw_action={action} safe_action={safe_action}"
            )
            self._debug(
                "obstacle_guard block_forward "
                f"context={context} center_depth={self._format_float(center_depth, 3)} "
        ...
        self._log_event(
            "obstacle_guard allow_forward "
            f"context={context} center_depth={self._format_float(center_depth, 3)} "
            f"left_depth={self._format_float(left_depth, 3)} "
            f"right_depth={self._format_float(right_depth, 3)} "
            f"likely_target={likely_target} raw_action={action}"
        )
        self._debug(
            "obstacle_guard allow_forward "
            f"context={context} center_depth={self._format_float(center_depth, 3)} "
